...
import type { BranchCode, Recommendation, Snapshot, ThresholdSettings } from '../lib/types';
...
import { buildRecommendations, compareSnapshots, type TrendRow } from '../lib/analytics';
...
export function useAppData() {
  ...
  const addSnapshotFromFile = useCallback(
    async (file: File, dateOverride?: string) => {
      setUploadError(null);
      try {
        const rows = await parseTopModelsFile(file);
  ...
  const recommendations: Recommendation[] = useMemo(() => {
    ...
    return buildRecommendations(selectedSnapshot.rows, thresholds).map((r) => ({
      ...r,
